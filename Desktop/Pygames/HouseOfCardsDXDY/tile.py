...
class Platform(pygame.sprite.Sprite):
	...
	def collide_platforms(self):
		...
		for platform in self.room.platform_sprites:
			if platform.hitbox.colliderect(self.room.player.hitbox): 
				if self.room.player.hitbox.bottom <= platform.hitbox.top + 20 and self.room.player.vel.y > 0:
					self.room.player.hitbox.bottom = platform.hitbox.top
					self.room.player.vel.y = 0
					self.room.player.cyote_timer = 0
					self.room.player.on_ground = True
					self.room.player.on_wall = False
					self.room.player.on_platform = True
			else:
				self.room.player.on_platform = False

	def move(self):
		self.timer += 1
		if self.timer <= 300:
			self.vel.y = 0
			self.vel.x = self.room.moving_platform_speed
			self.room.player.moving_right_on_platform = True

		elif self.timer >300 and self.timer <=600:
			self.vel.x = 0
			self.vel.y = 1
			self.room.player.moving_down_on_platform = True
			self.room.player.moving_up_on_platform = False
			self.room.player.moving_right_on_platform = False
			
		elif self.timer >600 and self.timer <= 900:
			self.vel.y = 0
			self.vel.x = -self.room.moving_platform_speed
			self.room.player.moving_left_on_platform = True
			self.room.player.moving_down_on_platform = False
		elif self.timer > 900 and self.timer <= 1200:
			self.vel.x = 0
			self.vel.y = -1
			self.room.player.moving_up_on_platform = True
			self.room.player.moving_down_on_platform = False
			self.room.player.moving_left_on_platform = False
		else:
			self.timer = 0
			self.room.player.moving_right_on_platform = False
			self.room.player.moving_left_on_platform = False
...
class TrickPlatform(AnimatedTile):
	...
	def collide_platforms(self):
		#collided = pygame.sprite.spritecollide(self.room.target, self.room.trick_platform_sprites, False)
		for platform in self.room.trick_platform:
			if platform.hitbox.colliderect(self.room.target.hitbox): 
				if self.room.target.hitbox.bottom <= platform.hitbox.top + 20 and self.room.target.vel.y >= 0:
					if platform.frame_index < 8:
						self.on_platform = True
						self.room.target.cyote_timer = 0
						self.room.target.vel.y = 0
						self.room.target.hitbox.bottom = platform.hitbox.top
...
class TrickPlatform2(TrickPlatform):
	def __init__(self, game, room, pos, groups, path):
		super().__init__(game, room, pos, groups, path)
	...
	def collide_platforms(self):
		#collided = pygame.sprite.spritecollide(self.room.target, self.room.trick_platform_sprites, False)
		for platform in self.room.trick_platform_2:
			if platform.hitbox.colliderect(self.room.target.hitbox): 
				if self.room.target.hitbox.bottom <= platform.hitbox.top + 20 and self.room.target.vel.y >= 0:
					if platform.frame_index < 8:
						self.on_platform = True
...
class TrickPlatform3(TrickPlatform):
	def __init__(self, game, room, pos, groups, path):
		super().__init__(game, room, pos, groups, path)
	...
	def collide_platforms(self):
		#collided = pygame.sprite.spritecollide(self.room.target, self.room.trick_platform_sprites, False)
		for platform in self.room.trick_platform_3:
			if platform.hitbox.colliderect(self.room.target.hitbox): 
				if self.room.target.hitbox.bottom <= platform.hitbox.top + 20 and self.room.target.vel.y >= 0:
					if platform.frame_index < 8:
						self.on_platform = True

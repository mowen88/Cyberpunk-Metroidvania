...
class Entity(pygame.sprite.Sprite):
	def __init__(self, game, room, char, pos, groups, obstacle_sprites):
		...
		self.moving_right_on_platform = False
		self.moving_left_on_platform = False
		self.moving_up_on_platform = False
		self.moving_down_on_platform = False
	...
	def moving_left_and_right(self):
		if self.alive and not self.dashing and not self.wall_kicking and not self.on_platform:
			...
			if self.moving_right_on_platform:
				if self.moving_right and self.vel.x >= self.room.moving_platform_speed:
					self.vel.x += self.acceleration
					self.facing = 1
					
				elif self.moving_left and self.vel.x <= self.room.moving_platform_speed:
					self.vel.x -= self.acceleration
					self.facing = -1
			
				elif self.facing == 1:
					self.vel.x -= self.friction
					if self.vel.x <= self.room.moving_platform_speed:
			...
			elif self.moving_left_on_platform:
				if self.moving_right and self.vel.x >= -self.room.moving_platform_speed:
					self.vel.x += self.acceleration
					self.facing = 1
			...
			self.moving_right_on_platform, self.moving_left_on_platform = False, False
			...
			if self.dashing:
				...
				if self.on_platform:
					if not self.crouching:
						if self.moving_right_on_platform:
							if self.vel.x == self.room.moving_platform_speed or self.on_right or self.on_left:
	...
	def apply_acceleration(self):
		if self.on_platform:
			if self.moving_right_on_platform:
				if self.vel.x >= self.speed + self.room.moving_platform_speed:
					self.vel.x = self.speed + self.room.moving_platform_speed
				if self.vel.x <= - self.speed + self.room.moving_platform_speed:
					self.vel.x = - self.speed + self.room.moving_platform_speed
